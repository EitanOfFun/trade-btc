...
Elm.Exchange = Elm.Exchange || {};
Elm.Exchange.make = function (_elm) {
   "use strict";
   _elm.Exchange = _elm.Exchange || {};
   if (_elm.Exchange.values) return _elm.Exchange.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var toStringText = function (val) {    return $Html.text($Basics.toString(val));};
   var val = A2($Json$Decode.at,_U.list(["ticker","last"]),$Json$Decode.string);
   var parseLastPrice = function (json) {
      var _p0 = A2($Json$Decode.decodeString,val,json);
      if (_p0.ctor === "Ok") {
            return _p0._0;
         } else {
            return "";
   ...
   _U.list([]),
   _U.list([A2($Html.a,_U.list([$Html$Attributes.href("https://www.eitansternlicht.com")]),_U.list([$Html.text("Created By Eitan Sternlicht")]))]));
   var pageHeader = A2($Html.h1,_U.list([]),_U.list([$Html.text("Convert BTC")]));
   var view = F2(function (address,model) {    return A2($Html.div,_U.list([$Html$Attributes.id("container")]),_U.list([pageHeader,pageFooter]));});
   var lastPrice = Elm.Native.Port.make(_elm).inboundSignal("lastPrice",
   "String",
   function (v) {
      return typeof v === "string" || typeof v === "object" && v instanceof String ? v : _U.badPort("a string",v);
   });
   var main = A2($Signal.map,$Html.text,A2($Signal.map,parseLastPrice,lastPrice));
   var update = F2(function (action,model) {
      var _p1 = action;
      switch (_p1.ctor)
      {case "NoOp": return model;
         case "UpdateRightInput": return model;
         default: return model;}
   });
   var UpdateRightInput = {ctor: "UpdateRightInput"};
   var UpdateLeftInput = {ctor: "UpdateLeftInput"};
   var NoOp = {ctor: "NoOp"};
   var initialModel = {leftInput: "",rightInput: ""};
   var Model = F2(function (a,b) {    return {leftInput: a,rightInput: b};});
   return _elm.Exchange.values = {_op: _op
                                 ,Model: Model
                                 ,initialModel: initialModel
                                 ,NoOp: NoOp
                                 ,UpdateLeftInput: UpdateLeftInput
